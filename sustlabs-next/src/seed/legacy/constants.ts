/**
 * Frozen snapshot of the site's original content, used only as the seed's input.
 * Its shapes come from ./types, which is the pre-CMS type file — that keeps this
 * decoupled from src/types/index.ts, which now describes what the components
 * receive from Payload and has moved on (images are objects, not lookup keys).
 *
 * Do not edit to change the live site. After a page is wired to Payload, its
 * content is edited in the admin panel.
 */
import type { FmsPageData, MonitoringPageData, SmartDbPageData } from './types'

const fmsBrochure = '/assets/sustlabs-acms-brochure-2026.pdf'
const smartDbBrochure = '/assets/sustlabs-smart-db-brochure.pdf'
const panasonicLogo = '/assets/logo-panasonic.png'
const schneiderLogo = '/assets/logo-schneider.png'
const ineproLogo = '/assets/logo-inepro.png'
